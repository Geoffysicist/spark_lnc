...
// 5. Rate of spread for zero wind, 'R0' (input), recommended as 5 m/min. - not used?
...
if (wind_speed < 5){
    head_speed = (R0 + 0.2*(5.67*pow(5*WF, 0.91) - R0)*wind_speed) * pow(Fuel_ht, 0.22) * exp(-0.076*MC);
} else {
    head_speed = 5.67 * pow(WF*wind_speed, 0.91) * pow(Fuel_ht, 0.22) * exp(-0.076*MC); }
